...
from neuralnet.unet.unet_dataloader import PatchesGeneratorPerImgObj
...
class UNetNNTrainer(NNTrainer):
    ...
    def _evaluate(self, dataloader=None, force_checkpoint=False):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            self._log(
                ','.join(str(x) for x in [1, self.checkpoint['epochs'], i + 1] +
                         ScoreAccumulator().add(labels, predicted).get_prf1a()))
            p, r, f1, a = score_acc.add(labels, predicted)
        ...
        if not isinstance(dataloader.dataset, PatchesGeneratorPerImgObj):
